const User = require('../models/user.model') // for importing the user model for performing the operations on the user
...
module.exports.authUser = async (req, res, next) => {
    ...
    try {
        ...
        const user = await User.findById(decoded._id); // for finding the user in the database by using the user id which we got from the token after decoding. and we have to find the user because we have to attach the user to the request object and then we will call the next function for moving to the next middleware or controller.
        ...
        req.user = user; // mean's request object ma user(user_id) attach kar diya taki hum next middleware ya controller ma user(user_id) ko access kar sake.
